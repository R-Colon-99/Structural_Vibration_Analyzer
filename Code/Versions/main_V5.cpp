// V5 - 240 Hz optimized auto-detected MPU6050 binary vibration logger
...
//   I2C clock:        400 kHz
//   Output rate:      240 Hz exact long-term average
//   MPU internal rate: 250 Hz
//   MPU6050 DLPF:     CFG=2 (~94 Hz accel, ~98 Hz gyro)
//   Accelerometer:    +/-8 g
//   Gyroscope:        +/-250 deg/s
//   Serial:           921600 baud
//
...
// ESP32 output stream target.
// 1,000,000 / 240 = 4166.666... us, so a rational microsecond
// scheduler is used instead of truncating the period to 4166 us.
const uint32_t SAMPLE_RATE_HZ = 800;
const uint32_t SAMPLE_PERIOD_BASE_US = 1000000UL / SAMPLE_RATE_HZ;  // 1250 us
const uint32_t SAMPLE_PERIOD_REMAINDER = 1000000UL % SAMPLE_RATE_HZ; // 160

// MPU6050 internal sample rate with DLPF enabled:
// 1 kHz / (1 + SMPLRT_DIV) = 250 Hz.
const uint32_t MPU_INTERNAL_RATE_HZ = 400;
...
// 500 samples at the MPU internal 250 Hz rate is approximately 2 seconds.
const int CALIBRATION_SAMPLES = 500;

// MPU6050 register configuration.
const uint8_t DLPF_CFG = 0x02;       // ~94 Hz accel BW, ~98 Hz gyro BW
const uint8_t GYRO_CONFIG = 0x00;    // +/-250 deg/s
const uint8_t ACCEL_CONFIG = 0x10;   // +/-8 g
const uint8_t SMPLRT_DIV = 0x03;     // 1 kHz / (1 + 3) = 250 Hz
...
void setup() {
  ...
  Serial.println("# ESP32 AUTO-DETECT MPU6050 PCA9548A binary logger - V5");
  Serial.print("# Baud: ");
  ...
  Serial.print("# MPU internal sample rate: ");
  Serial.print(MPU_INTERNAL_RATE_HZ);
  Serial.println(" Hz");
  Serial.println("# Accel bandwidth: approximately 94 Hz");
  Serial.println("# Gyro bandwidth: approximately 98 Hz");
  Serial.println("# Output Nyquist frequency: 120 Hz");
...
void loop() {
  // 240 Hz cannot be represented by one integer-microsecond period:
  // 1,000,000 / 240 = 4166.666... us.
  //
  // This scheduler alternates 4166/4167 us intervals so the long-term
  // output rate is exactly 240 Hz without accumulating timing drift.
  ...
  if ((int32_t)(now - nextSampleUs) >= 0) {
    ...
    // Add one nominal 4166 us period.
    nextSampleUs += SAMPLE_PERIOD_BASE_US;

    // Distribute the fractional 0.666... us across successive samples.
