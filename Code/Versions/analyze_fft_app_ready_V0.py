# V4 - App-ready analyzer for Structural Vibration Analyzer
...
# Use a non-interactive backend so the script works cleanly from the PC app.
matplotlib.use("Agg")
...
# ============================================================
# COMMAND-LINE ARGUMENTS / FILE SELECTION
# ============================================================

def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze ESP32 MPU6050 vibration CSV data and save FFT/time-domain results."
    )
    parser.add_argument(
        "--input",
        default=None,
        help="Full path to the raw CSV file to analyze. If omitted, the script lets you select from RAW_DATA_DIR.",
    )
    parser.add_argument(
        "--output-dir",
        default=str(PROCESSED_DIR),
        help="Folder where the processed run folder will be created.",
    )
    parser.add_argument(
        "--raw-dir",
        default=str(RAW_DATA_DIR),
        help="Folder to search when --input is omitted.",
    )
    parser.add_argument(
        "--file-pattern",
        default=FILE_PATTERN,
        help="Raw CSV file pattern to search when --input is omitted.",
    )
    parser.add_argument(
        "--show-plots",
        action="store_true",
        help="Show plots interactively after saving them. Not recommended when launched from the PC app.",
    )
    return parser.parse_args()


def select_csv_file(raw_data_dir: Path, file_pattern: str) -> Path:
...
if args.input:
    ...
    if not CSV_FILE.exists():
        raise FileNotFoundError(f"Input CSV file was not found: {CSV_FILE}")
else:
    CSV_FILE = select_csv_file(RAW_DATA_DIR, FILE_PATTERN)
...
run_folder = PROCESSED_DIR / f"{raw_stem}_processed_V3"

if run_folder.exists():
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    run_folder = PROCESSED_DIR / f"{raw_stem}_processed_V3_{timestamp}"
