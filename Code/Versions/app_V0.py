...
# ============================================================
# Save this file as:
# F:\RLenovo99\Documents\Projects\Structural_Vibration_Analyzer\Code\PC_APP\app.py
#
# Existing tools location:
# F:\RLenovo99\Documents\Projects\Structural_Vibration_Analyzer\Code\python_tools
# ============================================================
...
def choose_raw_file():
    """Browse for a raw CSV file to analyze."""
...
def validate_python_tools():
    """Check if the expected analyzer and logger scripts exist."""
    missing = []
...
def start_logger():
    """
    Start the logger script as a separate Python process.

    This app tries to call logger_esp32.py using useful command-line arguments:
        --port
        --output
        --baud

    If your logger_esp32.py does not currently accept these arguments,
    you will need to update logger_esp32.py later.
    """
    ...
    try:
        ...
        set_status(f"Logger running. Saving to: {output_file.name}")
        log_to_console(f"\n--- Logger started ---\n")
...
def run_analyzer():
    """
    Run analyze_fft.py on the selected raw data file.

    This app tries to call analyze_fft.py using:
        --input
        --output-dir

    If your analyze_fft.py does not currently accept these arguments,
    you will need to update analyze_fft.py later.
    """
...
def wait_for_analyzer():
    """Wait for analyzer to finish and update status."""
    global analyzer_process
...
def log_to_console(text):
    """Write text to the app console box."""
...
def read_process_output(process, label):
    """Read stdout/stderr from a subprocess and show it in the console."""
    try:
